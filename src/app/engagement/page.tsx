  ...
  return (
    <DocPage
      ...
      evidence={{
        ...
        title: "Fictional engagement. Local AvalancheGo ran. No regulated customer.",
        note: "Operate, interoperate, and a second overlay have local artifacts. AWS apply, Fuji, and hardware HSM have not.",
      }}
    >
      ...
      <Section title="How this engagement should be run">
        <ol className="list-decimal space-y-3 pl-5">
          <li>
            <strong className="text-white">Understand.</strong> Run AvalancheGo locally. Touch the EVM. Read the P-Chain as the validator registry. Add and remove validators by hand before automating anything. Status: locally executed. See evidence/runs/20260824T202726Z.
          </li>
          <li>
            <strong className="text-white">Operate.</strong> Stand up validators. Break one. Fence the old host. Restore the same NodeID. Watch accepted height. Status: locally executed on a 1-of-1 L1. Console destroy/restore is still JSON.
          </li>
          <li>
            <strong className="text-white">Interoperate.</strong> Deploy Teleporter, run a relayer, send AssetApproved, and verify ApprovalReceived with origin checks. Status: live delivery recorded. Foundry still covers auth.
          </li>
          <li>
            <strong className="text-white">Secure.</strong> Threat-model the system. Segment validator and RPC networks. Write down key custody, including the HSM design that is not faked. Status: local Envoy/signer/backup/observe substitutes. AWS and HSM are not implemented.
          </li>
          <li>
            <strong className="text-white">Automate.</strong> Only then Terraform, Ansible, and operator scripts that talk to real nodes.
          </li>
          <li>
            <strong className="text-white">Productize.</strong> Split Northstar-specific policy from the kit. Meridian Clearing is that second overlay.
          </li>
        </ol>
      </Section>
      ...
      <Section title="What becomes the kit">
        <p>
          Network isolation, validator and RPC split, ValidatorManager lifecycle, staking-key backup, ICM wiring, observability, and the runbooks. Meridian Clearing is the second overlay. Terraform apply has not been run.
        </p>
      </Section>
    </DocPage>
  );
